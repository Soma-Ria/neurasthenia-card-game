export class GameRoom {
 ...
 async handle(ws,m){try{
  ...
  if(m.type==="join"&&!me){if(this.data.phase!=="lobby")throw Error("游戏已经开始，暂时不能加入");if(this.data.players.length>=6)throw Error("房间已满（最多6人）");this.addPlayer(m,false)}
...
export default {async fetch(req,env){const u=new URL(req.url);if(u.pathname.startsWith("/room/")){let parts=u.pathname.split("/");let code=parts[2];if(!code)return new Response("Missing room code",{status:400});return env.ROOM.get(env.ROOM.idFromName(code.toUpperCase())).fetch(req)}return env.ASSETS.fetch(req)}}