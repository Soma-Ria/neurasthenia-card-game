...
const RULES=`<h3>普通规则</h3><p>轮到玩家时翻开两张牌。相同组成一对，<b>+1 分</b>并继续；不同则不加分并换下一位。全部配对完成后，分数最高者获胜。</p><h3>角色规则</h3><p>普通牌的一对：0 分；其他玩家所属的一对：<b>+1 分</b>；自己所属的一对：<b>-2 分</b>。所有分数变化都按“一对牌”计算。</p><h3>鬼牌</h3><p>翻开鬼牌后，已获得的卡牌归属和分数按设置的顺时针或逆时针方向转动。场上只剩最后一张牌时立即结束。</p><h3>联机</h3><p>一个房间最多 6 人。房主负责卡牌和规则设置。每位玩家可以选择自己的颜色，颜色不能重复。</p>`;
...
function connect(c,create=false){code=c;const p=location.protocol==="https:"?"wss":"ws";ws=new WebSocket(`${p}://${location.host}/room/${c}/ws`);
...
ws.onmessage=e=>{const m=JSON.parse(e.data);if(m.type==="state"){state=m.state;render()}if(m.type==="error")alert(m.message)};ws.onclose=()=>$("#status").textContent="连接断开"}
...
$("#rule").disabled=$("#joker").disabled=$("#direction").disabled=!host;$("#editor").classList.toggle("hidden",!host);
