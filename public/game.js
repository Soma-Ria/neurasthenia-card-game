...
const RULES=`<h3>游戏目标</h3><p>记住牌的位置，尽可能多地找到相同的一对。普通规则比谁得分高；角色规则除了得分，还要留意哪些牌属于自己或其他玩家。</p><h3>怎么玩？</h3><ol><li>房主先在准备阶段设置规则和卡牌，所有玩家选择自己的名字与颜色。</li><li>所有玩家都点击“准备”后，房主才能开始游戏。</li><li>轮到你时，依次翻开两张还没有配对的牌。</li><li>如果两张牌是同一组，就算配对成功；这两张牌会保留翻开状态，你获得对应分数，并继续自己的回合。</li><li>如果两张牌不同，会按照房主设置的时间保持翻开，然后同时盖回，并把回合交给下一位玩家。</li></ol><h3>普通规则</h3><p>每成功配对一组牌，获得 <b>1 分</b>。所有牌都配对完成后，分数最高的玩家获胜。</p><h3>角色规则</h3><p>每一组牌可以指定所属角色。配对成功时：<b>普通牌不加分</b>；获得其他玩家所属的牌 <b>+1 分</b>；获得自己所属的牌 <b>-2 分</b>。分数按“一整对牌”计算，而不是按单张牌计算。</p><h3>鬼牌</h3><p>如果开启鬼牌，翻到鬼牌后，会按照房主选择的方向，让已经获得的角色归属和分数一起转动；随后所有尚未配对的牌都会<b>重新洗牌并重新盖上</b>。鬼牌本身也会重新进入牌堆。只要场上只剩最后一张未配对的牌，游戏就会立即结束。</p><h3>双人对战</h3><p>如果房间只有两名玩家，并且使用角色规则，当其中一名玩家所属的所有角色牌都已经被配对获得时，立即结束对战，由当前分数更高的一方获胜。</p><h3>联机说明</h3><p>一个房间最多 6 人。玩家颜色不能重复；刷新页面或短暂断线会尝试恢复原来的玩家身份，主动点击“返回首页/退出房间”才会离开房间。</p>`;
...
let pref=JSON.parse(localStorage.getItem("ng_pref")||"null")||{name:"玩家",color:0,rule:"normal",joker:"off",jokerCount:1,direction:"cw",cards:[{name:"猫",owner:"",count:2},{name:"狗",owner:"",count:2},{name:"鸟",owner:"",count:2},{name:"鱼",owner:"",count:2}]};
...
ws.onopen=()=>{if(create)history.replaceState({},"",`/room/${c}`);send(create?"create":"join",create?{code:c,settings:{rule:pref.rule,joker:pref.joker,jokerCount:+pref.jokerCount||1,direction:pref.direction,cards:pref.cards},name:pref.name,color:pref.color}:{name:pref.name,color:pref.color});$("#status").textContent="🟢 已连接"};
...
function showJokerNotice(m){$("#modalText").innerHTML=`<div style="text-align:center"><div style="font-size:42px">👻</div><h2>鬼牌触发！</h2><p>所有未配对的卡牌已重新洗牌并盖回。</p><p>已获得的卡牌与分数已按<b>${m.direction==="cw"?"顺时针":"逆时针"}</b>方向转动。</p></div>`;$("#modal").classList.remove("hidden");clearTimeout(window.jokerNoticeTimer);window.jokerNoticeTimer=setTimeout(()=>$("#modal").classList.add("hidden"),2200)}
...
$("#rule").value=state.settings.rule;$("#joker").value=state.settings.joker;$("#jokerCount").value=String(state.settings.jokerCount||1);$("#direction").value=state.settings.direction;$("#failTime").value=String(state.settings.failTime||2);$("#jokerSettings").classList.toggle("hidden",state.settings.joker!=="on");$("#rule").disabled=$("#joker").disabled=$("#direction").disabled=$("#failTime").disabled=!host;$("#editor").classList.toggle("hidden",!host);
$("#nameInput").value=me?.name||pref.name;$("#readyBtn").classList.toggle("ready",!!me?.ready);$("#readyBtn").textContent=me?.ready?"✓ 已准备（点击取消）":"○ 尚未准备（点击准备）";
...
function roleStyle(c){if(!c.owner)return"";let p=state.players.find(x=>x.id===c.owner);if(!p)return"";let col=COLORS[p.color][0];return ` style="--role:${col};--role-bg:${col}66"` }
...
for(const [sel,key] of [["#rule","rule"],["#joker","joker"],["#jokerCount","jokerCount"],["#direction","direction"],["#failTime","failTime"]])$(sel).onchange=e=>{pref[key]=e.target.value;save();send("settings",{settings:{...state.settings,[key]:e.target.value}})};
